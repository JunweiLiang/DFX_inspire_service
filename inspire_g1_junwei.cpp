...
public:
  ...
  {
    // modified by junwei
    // now we can have both hands working
    //serial1 = std::make_shared<SerialPort>("/dev/ttyUSB1", B115200);
    //serial2 = std::make_shared<SerialPort>("/dev/ttyUSB2", B115200);
    serial_left = std::make_shared<SerialPort>(param::serial_port_left, B115200);
    serial_right = std::make_shared<SerialPort>(param::serial_port_right, B115200);

    // If your left and right hand controls are reversed, you can swap the positions of `serial1` and `serial2` below.
    righthand = std::make_shared<inspire::InspireHand>(serial_right, 1);
    lefthand = std::make_shared<inspire::InspireHand>(serial_left, 1);
  ...
  SerialPort::SharedPtr serial_left;
  SerialPort::SharedPtr serial_right;
